...
{
    for (auto& l : mInputLayouts)
    {
        if (shader->GetShaderData().vertexLayout == l.second.LayoutKioto)
            mInputLayouts[shader->GetHandle()] = l.second; // [a_vorontcov] TODO: remove copy.
    }
