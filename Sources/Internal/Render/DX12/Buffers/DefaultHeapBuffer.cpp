...
DefaultHeapBuffer::DefaultHeapBuffer(const byte* data, uint32 dataSize, ID3D12GraphicsCommandList* commandList, ID3D12Device* device, D3D12_RESOURCE_STATES destinationState)
{
    ...
    mUploadBuffer->Map(0, &range, reinterpret_cast<void**>(&mappedBuffer));
    ...
    CD3DX12_RESOURCE_BARRIER toDest = CD3DX12_RESOURCE_BARRIER::Transition(mBuffer.Get(), D3D12_RESOURCE_STATE_COPY_DEST, destinationState);
